...
└── files/
    ...
    └── usr/share/reyos/reader/
        ...
        │   └── pdf_annotate.py # PyMuPDF-backed highlight/free-text/comment writer + lister, see "PDF find/highlight/annotations" below
        └── qml/
            ├── qmldir
            ├── ReyOSStyle.qml
            ├── Main.qml
            ...
            ├── BookReaderPage.qml
            ├── PdfReaderPage.qml
            ├── PdfAnnotatedMultiPageView.qml  # forked QtQuick.Pdf PdfMultiPageView.qml + an annotation overlay, see below
            ├── ComicReaderPage.qml
            ├── PdfToolsPage.qml
...
| PDF highlight/free-text/comment annotations | `python-pymupdf` | pip `pymupdf` in the venv (imported as `pymupdf`, not the deprecated `fitz` alias) |

`reyos-reader`'s Arch `PKGBUILD` `depends=()` (as of pkgrel 34):
```
depends=('pyside6' 'qt6-webengine' 'qt6-webchannel' 'python-ebooklib' 'python-rarfile' 'unrar' 'python-pypdf' 'python-pymupdf')
```

### Ubuntu/Debian packaging (added 2026-09-10)

Same self-contained-venv `.deb` shape as `reyos-browser`'s: a bundled Python venv under `/opt/reyos-reader/.venv` (built by copying the browser's already-working PySide6+QtWebEngine venv, then `pip install ebooklib rarfile pypdf pymupdf` into the copy — avoids a from-scratch PySide6 download), `/usr/bin/reyos-reader` launcher pointing at it, `/usr/share/applications/reyos-reader.desktop`. `apt` `Depends:` covers `unrar` plus the same Chromium/QtWebEngine runtime library set the browser's `.deb` needed (`libnspr4`, `libnss3`, `libxcb-cursor0`, etc. — Reader also embeds a `WebEngineView` for EPUB).

**Real gap, same class already flagged for the browser**: the staging directory (`~/reyos-reader-deb`) lives entirely outside git, same as `~/reyos-browser-deb` — not version-controlled, so a future rebuild has no reviewable source of truth for `DEBIAN/control`/`postinst`. Still not brought into the repo (explicitly deferred for the browser's version 2026-09-10; the same call has applied here through many more version bumps since — worth revisiting given how much churn this directory has now seen).

**Gotcha hit twice packaging into this exact venv (2026-09-12)**: the venv's own `bin/pip`/`bin/python3.12` scripts have a hardcoded shebang baked in at creation time (`#!/opt/reyos-browser/.venv/bin/python3` — this venv was originally created by copying the *browser's* venv, path and all, never updated). Calling `.venv/bin/pip install X` directly runs the wrong interpreter and silently tries to install into `/opt/reyos-browser/.venv` instead. Always invoke as `.venv/bin/python3 -m pip install X` — `bin/python3` itself is a plain symlink to the real system `/usr/bin/python3` and resolves `sys.prefix` correctly regardless of the directory's history; only the `pip`/`python3.12` **scripts** carry the stale shebang text.

As of 2026-09-12, published through `reyos-reader-v1.0.0-17ubuntu1` on `Reypaulino/ReyOS` GitHub Releases (only the single newest Reader release is kept there — old ones are deleted as each new one is created, see the package-publish-pipeline reference). See "PDF find/highlight/annotations" and the pagination fix below for what's actually in the current build; too many incremental version bumps happened in a single session to itemize each one usefully here.
...
- EPUB highlighting (2026-09-10) uses a DOM `<mark>`-wrapping approach specific to the `WebEngineView`. PDF highlighting (2026-09-12, see "PDF find/highlight/annotations" below) is a genuinely different mechanism — real PDF annotations written via PyMuPDF, not DOM marks — so the two features share no code, just the same user-facing idea. Comic pages (CBZ/CBR/images) still have neither; they render via a plain `Image` bound to an image provider, no DOM and no PDF annotation model to target. See Roadmap.
...
## Paginated EPUB reading mode (added 2026-09-12)

User asked for a way to "cut the scrolling" while reading — a page-by-page mode as an alternative to `BookReaderPage.qml`'s default continuous vertical scroll, closer to a physical book/Kindle's page-flip. A header toggle button (`Scroll` / `Pages`) switches `page.paginated`; when on, injected CSS turns the chapter's `<body>` into fixed-width, fixed-height CSS multi-columns (`column-fill: auto`, one column exactly one screen wide/tall), so what used to be vertical scroll position becomes horizontal `scrollLeft`, one "page" per column. `nextPage()`/`prevPage()` step by one column width, falling through to the adjacent chapter only at the first/last column.

**This looked simple and took three real, wrong fixes before the actual bug was found** — worth reading in full before touching this code again:

1. First bleed-through report: content from the next page visibly overlapping the current one. Diagnosed (wrongly, first pass) as a `column-width` unit mismatch (`100vw` vs the true rendered width, since `vw` doesn't always account for scrollbar-gutter the same way) — fixed to compute `column-width` from an exact pixel measurement instead. Bug persisted.
2. Second pass found a real, separate, but not-yet-the-actual-cause issue: every scroll function used `document.scrollingElement` to read/set `scrollLeft`/`scrollWidth`. Once `<html>` itself has `overflow: hidden` (needed for pagination), Chromium clamps `<html>`'s own reported `scrollWidth` to its `clientWidth` — i.e. it reports *zero* overflow — even though its child `<body>` (the actual wide, multi-column element) correctly reports the true `scrollWidth`. Every boundary check was therefore comparing against a `scrollWidth` that always equaled one page, so `nextPage()` looked "at the end of the chapter" immediately. Fixed by switching every read/write from `document.scrollingElement` to `document.body` directly. This was a real, necessary fix — but the bleed-through bug the user kept reporting was still there afterward, because scrollLeft assignments against the wrong (non-functional) element were simply having no effect; the visible symptom had a different cause than this fix addressed.
3. Third pass found a second real, necessary fix: the reading-margin setting was applied as `padding-left`/`padding-right` directly on `body`. Multi-column layout consumes that padding from the column *content* width, while the `overflow: hidden` clipping viewport stays at `body`'s full padding-box width — two different widths for what has to be the same measurement, so the actual rendered column ends up narrower than the viewport, letting the next column's content peek through on the right edge. Fixed by moving the margin onto `html`'s padding (whose own `clientWidth` is the fixed viewport size, unaffected by its own padding) and zeroing it on `body`, so `body`'s `clientWidth` — and thus its column width — shrinks to exactly the space actually available.
4. **The real, primary cause**, found only after live debugging via Chrome DevTools Protocol against the actual running app (screenshotting the live DOM, including via Chromium's own internal `Page.captureScreenshot` to rule out an OS/Qt-side paint-staleness theory that turned out to be a dead end): `restoreScroll(frac)` computed the target horizontal scroll position as `frac * (scrollWidth - pageWidth)` directly. That's essentially never an exact multiple of one page's width, so it lands the viewport straddling a column boundary — showing the tail of one column and the head of the next simultaneously. This is indistinguishable from "next page bleeding in" and was triggered on *every* Scroll/Pages mode toggle and every chapter/app open that restores a saved reading position (i.e., very often, and in a way that looked like the bug was never actually fixed even after 1–3 above genuinely were). Fixed by snapping to the nearest whole page: `Math.round((d * frac) / w) * w`. Verified live, repeatedly, across fresh opens, page turns, chapter boundaries, and repeated mode toggling — the exact trigger condition for the bug.

**Lesson for next time**: when a user keeps reporting the same symptom after a fix that is itself correct and necessary, don't assume the fix "didn't take" or look for a paint/caching explanation before re-deriving the actual arithmetic/logic path that produces the visible symptom. Two of the four rounds above were real, correct fixes for real problems that simply weren't *this* problem.

## PDF find/highlight/annotations (added 2026-09-12)

User asked, in the same spirit as EPUB highlighting, for the ability to find words in a PDF and highlight/annotate/comment on them. Three features, all in `PdfReaderPage.qml`:

- **Find in document** — a search bar bound to `PdfMultiPageView`'s own `searchString`/`searchModel` (`QPdfSearchModel`); match highlighting and next/prev navigation (`searchForward()`/`searchBack()`) are Qt built-ins, no custom code needed here.
- **Highlight selected text** — `view.selectedText` (Qt's own text-selection API) + `view.currentPage`, sent to a new `formats/pdf_annotate.py` (PyMuPDF-backed, distinct from `pdf.py`'s read-only `QPdfDocument` metadata helper and `pdf_tools.py`'s pypdf-backed merge/split — neither of those can write annotations). `add_highlight()` locates the text via `page.search_for(text, quads=True)` (trying a whitespace-collapsed version too, since a Qt selection spanning a line wrap often joins lines with different whitespace than PyMuPDF's own text extraction) and calls `add_highlight_annot()`. Chose PyMuPDF specifically because it can locate exact on-page rectangles for a text string, avoiding the need for pixel-perfect selection geometry from `QtQuick.Pdf`'s internals.
- **Add Text / Add Comment** — `FreeText` and `Text`(sticky-note/`Popup`) annotations placed via a click-to-place dialog: a `Popup` renders the current page via `PdfPageImage` (a plain `Image` subclass) at a size the dialog controls, so converting a click position to a page-point position is simple, known math (`click / (renderedWidth / pagePointWidth)`) rather than reverse-engineered from the live scrolling/zooming `PdfMultiPageView`.

New `python-pymupdf` dependency (see Dependencies above). Writes go to a temp file then `os.replace()` over the original (same atomic pattern used elsewhere in Reader), since `QPdfDocument` may still have the file open for reading in the viewer.

**Real bug found during this feature, not by review**: `meta.path` (as seen by QML) is a `file://` URL (`QUrl.fromLocalFile(...)`, needed for `PdfDocument.source`), not a plain filesystem path — the new backend annotation Slots need `QUrl(path).toLocalFile()` before handing it to PyMuPDF, or every write fails with a "could not open" error pointing at a URL-shaped string that doesn't exist as a file. Two smaller real bugs also found live: a double-`close()` on the PyMuPDF `Document` after each write (`_save_over()` already closes it; querying even `.is_closed` on an already-closed `Document` itself raises, so the caller's own cleanup path needs a `saved` flag instead of an unconditional `finally`-close); and `add_freetext_annot()`'s text-color keyword is actually named `text_color` in the installed PyMuPDF version, not `color`.

### Getting annotations to actually show up in the app (added 2026-09-12, same day)

Shipping the above, the user immediately reported (correctly) that an added comment wasn't visible. Root cause: **`QtQuick.Pdf`'s `PdfMultiPageView`/`PdfDocument` render only page *content*, never PDF annotations, at all** — confirmed live: a freshly-saved highlight or comment was invisible in Reader's own view even though it was a correctly-placed, real annotation that PyMuPDF's own `page.get_pixmap()` (and any other real PDF viewer) rendered correctly. This is a genuine QtQuick.Pdf module limitation, not a bug in the write path above.

Fixed by forking `PdfMultiPageView.qml` into this app's own `qml/PdfAnnotatedMultiPageView.qml` — explicitly the sanctioned customization path per that file's own upstream doc comment ("In case you want to make changes in your own version of this component, you can copy the QML ... and modify it as needed"), so this stays a small, reviewable diff against Qt's own ~600-line implementation rather than an from-scratch PDF view. The only real addition: a `annotations` property (a flat list of `{page, kind, x, y, width, height, text}`, populated from a new `backend.getPdfAnnotations()` Slot that lists every annotation across the whole document via PyMuPDF) and a `Repeater` per page delegate drawing a highlight tint, a bordered box for free text, or a small dot marker for comments — positioned the same way the existing `PdfLinkDelegate` repeater already is (page points × `paper.pageScale`). Tapping a marker emits `annotationTapped(annotation)`, which `PdfReaderPage.qml` uses to show the full text in a small popup (a hover tooltip alone wouldn't work for touch input, and can clip longer notes). `PdfReaderPage.qml` reloads `annotations` after the initial page load and after every annotation write (same reload-bounce that already existed for making a fresh write visible in the page content itself).

Highlight rendering is a deliberate approximation: it draws the annotation's overall bounding rect rather than reconstructing its precise per-line `QuadPoints` geometry — close enough to show "something is highlighted here," not pixel-perfect for a highlight spanning a line wrap. Good enough for the "can I see what I added" ask; revisit only if that approximation is ever visibly wrong in practice, not preemptively.
...
3. **PDF form-field filling.** Requested 2026-08-26, deferred since. `python-pymupdf` is now already a real dependency (2026-09-12, for highlight/free-text/comment annotations — see "PDF find/highlight/annotations" above), so check its form-field API first rather than reaching for a new library. Still needs QML input widgets overlaid on `PdfAnnotatedMultiPageView` at each field's page-space rect (the same page-point × `pageScale` positioning pattern the annotation overlay already uses). Scope save-in-place vs. save-as and encrypted-PDF interaction before starting; this is a real subsystem, not a small addition.
4. **Comic highlighting/annotation.** PDF got real highlight/free-text/comment annotations 2026-09-12 (see above); EPUB highlighting has existed since 2026-09-10. Comic pages (CBZ/CBR/images) still have neither — they render via a plain `Image` bound to an image provider, no DOM and no PDF annotation model to target. Would need a parallel, genuinely different mechanism from either existing one: an overlay `Canvas`/`Shape` drawing highlight rects in page-space coordinates, persisted as `(page, rect, color)` in a new table (or extending the existing `highlights` table with a page-rect column alongside its text-offset range). Real scope, don't bolt it onto either existing mechanism.
